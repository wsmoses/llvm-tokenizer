...
NUM_AGENTS = 1 
TRAIN_SEQ_LEN = 30  # take as a train batch
...
def agent(agent_id, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            #end_of_opt, video_chunk_remain = \
            #    net_env.get_video_chunk(selected_pass)

            # define the end_of_opt as a fixed number number of passes e.g 30 for now
            ...
            # Negative of Runtime times some other rewards e.g  Number of passes applied?  
            # Weight of some import passes 
            ...
            # -- log scale reward --
            # log_selected_pass = np.log(VIDEO_BIT_RATE[selected_pass] / float(VIDEO_BIT_RATE[-1]))
            # log_last_selected_pass = np.log(VIDEO_BIT_RATE[last_selected_pass] / float(VIDEO_BIT_RATE[-1]))

            # reward = log_selected_pass \
            #          - REBUF_PENALTY * rebuf \
            #          - SMOOTH_PENALTY * np.abs(log_selected_pass - log_last_selected_pass)

            # -- HD reward --
            # reward = HD_REWARD[selected_pass] \
            #          - REBUF_PENALTY * rebuf \
            #          - SMOOTH_PENALTY * np.abs(HD_REWARD[selected_pass] - HD_REWARD[last_selected_pass])
            ...
            if len(r_batch) >= TRAIN_SEQ_LEN or end_of_opt:
