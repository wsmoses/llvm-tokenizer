...
def passToStr(idx):
    print_pass = opts[usingopts[idx]] if idx < len(opts) else None
    if print_pass is not None:
        print(print_pass.getPassArgument() + " - " + print_pass.getPassName())
    else:
        print("Index out of bound")

def getTime(c_code, opt_indice, print_pass=False):
...
def getTimeOptLevel(c_code, level, print_pass=False):
    g = getPollyLLVM(c_code)
    success = True
    time = float('inf')

    if print_pass:
        print("Applying optimization level: " + level)
    success = applyOptLevel(g, level)
    if (not success):
        print("Failed to apply opt level")
        print(level)
    #print(g)
